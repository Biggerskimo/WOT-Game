					...
					<table border="1" width="100%">
						<tr>
							{foreach from=$data.attackerData key='fleetID' item='fleet'}
								...
								{assign var='userID' value=$fleetObj->ownerID}
								...
								<th>
									<br />
									<center>
										Angreifer {$userObj->username} ({$fleetObj->getStartPlanet()->galaxy}:{$fleetObj->getStartPlanet()->system}:{$fleetObj->getStartPlanet()->planet})
										<br>
										Waffen: {$userObj->military_tech*10}% Schilde: {$userObj->defence_tech*10}% H&uuml;lle: {$userObj->shield_tech*10}%
										<table border="1">
											{if !$shipNames|empty}
												<tr>
													<th>
						...
						<tr>
							{foreach from=$data.defenderData key='fleetID' item='fleet'}
								...
								<th>
									<br />
									<center>
										Verteidiger {$userObj->username} ({$fleetObj->galaxy}:{$fleetObj->system}:{$fleetObj->planet})
										<br>
										Waffen: {$userObj->military_tech*10}% Schilde: {$userObj->defence_tech*10}% H&uuml;lle: {$userObj->shield_tech*10}%
										<table border="1">
											{if !$shipNames|empty}
