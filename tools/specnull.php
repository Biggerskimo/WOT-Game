...
FROM `ugml_fleet` 
LEFT JOIN ugml_fleet_spec ON ugml_fleet.fleetID = ugml_fleet_spec.fleetID
WHERE ugml_fleet_spec.fleetID IS NULL";
...
while($row = WCF::getDB()->fetchArray($result)) {
	...
	$sql = "SELECT data
			...
			WHERE fleetID = ".$row['fleetID'];
	$row2 = WCF::getDB()->getFirstRow($sql);
	
	var_Dump($row2);
	$data = unserialize(LWUtil::unserialize($row2['data']));
	var_dump($data);
	...
	if($row['fleet_mess'] == 0) {
		$impactEvent = WOTEventEditor::create($classPath, $fleetID, array('state' => 0), $impactTime);
		$impactEventID = $impactEvent->eventID;
	}
	else {
		$impactEventID = 0;	
	}
	$returnEvent = WOTEventEditor::create($classPath, $fleetID, array('state' => 1), $returnTime);
	$returnEventID = $returnEvent->eventID;
	if($wakeUpTime) {
		$wakeUpEvent = WOTEventEditor::create($classPath, $fleetID, array('state' => 2), $wakeUpTime);
