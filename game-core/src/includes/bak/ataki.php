...
$atakujacy = attacker ships;
$wrog = defender ships;
$tech_atakujacy = attacker techs;
$tech_wrog = defender techs;
...
function walka($atakujacy,$wrog,$tech_atakujacy,$tech_wrog){
	...
	for ($i = 1; $i<=7;$i++){
		...
		/**
		 * Calulate hull platings (obrona; +- 20%), shields (tarcza; +- 20%), weapons (atak; +- 20%)
		 * and count them (atakujacy_* and wrog_*)
		 */
		...
		if (!is_null($wrog)) {
			...
			// if the defender is destroyed, collect data and end this round
			$wrog_ilosc = 0;
		...
		// end round if attacker or defender is destroyed
		if (($atakujacy_ilosc == 0) or ($wrog_ilosc == 0)){
			break;
		}

		// shots of the attacker
		foreach($atakujacy as $a => $b){
