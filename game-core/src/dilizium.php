...
if($_POST) {
	...
	// all
	$allDays = abs(intval($_POST['all']));
	$allCosts = $allDays * ALL;

	if($allCosts > $user['dilizium']) $allDays = floor($user['dilizium'] / ALL);
	$allCosts = $allDays * ALL;
	$user['lostDilizium'] += $allCosts;
	$user['dilizium'] -= $allCosts;

	foreach($allList as $elem)
	{
		if($features[$elem] < time()) $features[$elem] = (time() + $allDays * 24 * 60 * 60);
		else $features[$elem] += $allDays * 24 * 60 * 60;
	}
...
$page .= '<script type="text/javascript" src="game/js/jq.js"></script>
		<script type="text/javascript">
			...
			function calculateCosts(container) {
				...
				if(container == "all")
				{
					if(days > 0)
					{
						$(".notAll input").attr("disabled", "disabled");
						$(".notAll").addClass("disabled");
						
						// skip overall costs
						document.getElementById("costs").firstChild.data = costs;
						document.getElementById(container + "C").firstChild.data = costs;
						
						if(costs > document.getElementById("dilizium").firstChild.data) document.getElementById("costs").style.color = "red";
					...
					{
						$(".notAll input").removeAttr("disabled");
						$(".notAll").removeClass("disabled");
						old = 0;
					}
				}
				document.getElementById(container + "C").firstChild.data = costs;
...
$page .= '</tr>';
$page .= '<tr>';
$page .= '<td colspan="4" style="text-align: center;"><a href="game/index.php?page=Payment"><span style="color: gold;">Kaufen</span></a></td>';
$page .= '</tr>';
$page .= '<tr>';
