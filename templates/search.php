...
if(isset($_POST['searchtext']) && isset($_POST['type'])){
	while($r = mysql_fetch_array($search, MYSQL_BOTH)){
		...
		if($_POST['type']=='playername'||$_POST['type']=='planetname'){
			...
			{
				$pquery = doquery("SELECT {{table}}.*, ugml_alliance.ally_tag FROM {{table}} LEFT JOIN ugml_alliance ON ugml_alliance.id = {{table}}.ally_id WHERE {{table}}.id = {$s['id_owner']}","users",true);
				...
				$s['ally_name'] = ($s['ally_tag']!='')?"<a href=\"alliance.php?mode=ainfo&tag=".urlencode($s['ally_tag'])."\">".$s['ally_name']."</a>":'';
			}
			//ahora la alianza
